# Loki Mode - Gemini CLI Skill

Multi-agent autonomous startup system for Gemini CLI. Takes PRD to fully deployed, revenue-generating product with zero human intervention.

## Quick Start

```bash
# Launch Gemini CLI with autonomous permissions
...
references/                 # Detailed documentation (19 files)
  ...
  lab-research-patterns.md  # DeepMind + Gemini: Responsible AI, debate
  ...
  competitive-analysis.md   # Auto-Gemini, MemOS, Dexter comparison
  confidence-routing.md     # Model selection by confidence
  cursor-learnings.md       # Cursor scaling patterns
  prompt-repetition.md      # gemini-1.5-flash prompt optimization
...
### Model Selection
- **gemini-1.5-pro**: Planning and architecture ONLY (system design, high-level decisions)
- **gemini-3-flash-preview**: Development and functional testing (implementation, integration tests)
- **gemini-1.5-flash**: Unit tests, monitoring, and simple tasks - use extensively for parallelization
...
**Gemini:**
- Responsible AI (principles-based self-critique)
- Alignment Faking Detection (sleeper agent probes)
- Gemini CLI Best Practices (Explore-Plan-Code)
